...
struct SortedVectorInserter : public Comparer {
  std::pair<bool, typename std::vector<T>::iterator> LowerBound(std::vector<T>& el,
                                                                const T& value) {
    // GCC 13 ICE workaround: capturing `this` via [&] inside a generic lambda of a
    // template-dependent context triggers an internal compiler error on GCC 13
    // (ubuntu-24.04 CI). Bind the comparer explicitly instead; identical semantics.
...
struct PackageViewComparer {
  bool operator()(const ResourceTablePackageView& lhs,
                  const ResourceTablePackageView& rhs) const {
    return less_than_struct_with_name_and_id<ResourceTablePackageView, uint8_t>(
        lhs, std::make_pair(std::string_view(rhs.name), rhs.id));
  }
};

struct TypeViewComparer {
  bool operator()(const ResourceTableTypeView& lhs,
                  const ResourceTableTypeView& rhs) const {
    return lhs.id != rhs.id ? lhs.id < rhs.id : lhs.named_type < rhs.named_type;
  }
};

struct EntryViewComparer {
  bool operator()(const ResourceTableEntryView& lhs,
                  const ResourceTableEntryView& rhs) const {
